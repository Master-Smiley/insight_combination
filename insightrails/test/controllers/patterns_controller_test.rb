...
class PatternsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pattern" do
    assert_difference('Pattern.count') do
      post patterns_url, params: { pattern: { text: @pattern.text, user_id: @pattern.user_id } }
    end
  ...
  test "should update pattern" do
    patch pattern_url(@pattern), params: { pattern: { text: @pattern.text, user_id: @pattern.user_id } }
